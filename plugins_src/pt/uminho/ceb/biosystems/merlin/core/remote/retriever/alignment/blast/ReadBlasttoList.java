...
public class ReadBlasttoList {
	...
	public ReadBlasttoList(InputStream input) {
		...
		try {
			//			
			//			OutputStream os = new FileOutputStream("D:/blast_ncbi.txt");  
		...
		try {
			...
			if(cbf.isBlastResultOK()) {
				...
				if(cbf.isSimilarityFound()) {
					...
					//System.err.println("No similarity for gene ");
				}
			}
			else {

				//System.out.println("blast stream issue");
				this.setReprocessQuery(true);
			}
		}
		catch (Exception ex) {
			...
			//if(ex.toString().equals("java.util.NoSuchElementException: Property queryId unknown"))
			//System.out.println("A problem was found in the file: "+this.query);
			if(cbf.isSimilarityFound()) {
