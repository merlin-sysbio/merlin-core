...
public class DownloadNcbiFiles {
	...
	public void startDownload(boolean isGenBank) {
		...
		try {
			...
			CreateGenomeFile.saveAssemblyRecordInfo(docSum, database);
			...
			File faafastaFile = new File(FileUtils.getWorkspaceTaxonomyFolderPath(database, taxID) + FileExtensions.PROTEIN_FAA.getExtension());
			File fnafastaFile = new File(FileUtils.getWorkspaceTaxonomyFolderPath(database, taxID) + FileExtensions.CDS_FROM_GENOMIC.getExtension());
			...
			if(!faafastaFile.exists() && !fnafastaFile.exists())
				...
				CreateGenomeFile.createGenomeFileFromFasta(database, taxID, faafastaFile, FileExtensions.PROTEIN_FAA);
				Workbench.getInstance().info("only '.faa' " + ftpSource + " ftp file successfuly downloaded");
			}
			else if (!faafastaFile.exists()){
				CreateGenomeFile.createGenomeFileFromFasta(database, taxID, fnafastaFile, FileExtensions.CDS_FROM_GENOMIC);
