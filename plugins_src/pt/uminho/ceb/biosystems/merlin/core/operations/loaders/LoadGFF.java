		...
		if (ecOption && sourceOption.equals(GFFSource.Other)){
			for (int i = 0; i < entries.size(); i++)
			{
				if (cancel.get()) {
					...
					Workbench.getInstance().warn("GFF loading cancelled!");
					break;
				}
				...
				{
				DatabaseLoaders.loadGeneAnnotation(entrydata.getLocusTag(), genes.get(i)[6], temp, temp, genes.get(i)[3], genes.get(i)[1], genes.get(i)[2], entrydata.getEcNumbers(), temp, stmt, project, InformationType.GFF);
				}
			...
			for (int i = 0; i < entries.size(); i++)
			{
				...
				entrydata = entries.get(i);
				DatabaseLoaders.loadGeneAnnotation(entrydata.getLocusTag(), genes.get(i)[6], temp, temp, genes.get(i)[3], genes.get(i)[1], genes.get(i)[2], entrydata.getEcNumbers(), temp, stmt, project, InformationType.MANUAL);
